...
with tf.name_scope('inputs'):
    xs = tf.placeholder(tf.float32,[None,1],name='x_input')  #这个name的属性，也是为了使用tensorboard添上来的
...
#上面的with或者是name都是可选的，可以选择添加，也可以选择不添加，但是下面这一行是一定要写的。
#这个表明了 在当前的目录喜爱面创建以恶搞logs的文件夹，然后把图的信息保存进去
#这样运行完这段代码后，就会有一个logs的文件夹被创建
